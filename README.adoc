...
* Build docker image via `Dockerfile` (at the top level of your project)
    ...
    ENTRYPOINT ["java","-jar","/app.jar"]
        ...
        *** build the app's image
            **** via Maven
                ...
                docker build --build-arg JAR_FILE=target/docker-demo-0.0.1-SNAPSHOT.jar -t arguments/maven -f ./Dockerfiles/witharguments .

            **** via Gradle -

                docker build --build-arg JAR_FILE=build/libs/docker-demo-0.0.1-SNAPSHOT.jar -t arguments/gradle -f ./Dockerfiles/witharguments .
    ...
    ** without aguments == hard code the JAR location
        ...
        *** build the image
            ...
            docker build -t hardcoded/maven -f Dockerfiles/hardcodedmaven .
            docker build -t hardcoded/gradle -f Dockerfiles/hardcodedgradle .

* Run the docker image

    docker run -p 8080:8080 arguments/maven
    docker run -p 8080:8080 arguments/gradle
    docker run -p 8080:8080 hardcoded/maven
    docker run -p 8080:8080 hardcoded/gradle

* Trigger the sample requests, specified in 'samplerequests.http'
